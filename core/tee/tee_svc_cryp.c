...
#ifdef RCAR_STORAGE_KEY_BY_ICUMX_HWENGINE
TEE_Result syscall_icum_install_key(void *buf, size_t buf_len)
{
	if (!buf)
		return TEE_ERROR_BAD_PARAMETERS;

	if (!(buf_len % 16) && (buf_len > 31))
		return TEE_ERROR_BAD_PARAMETERS;
