...
const int8_t version_of_renesas[] __section(".version") =
	VERSION_OF_RENESAS;
...
void log_buf_init(void)
{
	...
	log_secram_header = (struct log_buf_header_t *)
		phys_to_virt(OPTEE_LOG_BASE, MEM_AREA_IO_SEC, MEMORY1_SIZE);
	 log_nonsec_ptr = (int8_t *)core_mmu_get_va(OPTEE_LOG_NS_BASE,
			MEM_AREA_RAM_NSEC, MEMORY2_SIZE);
...
void log_debug_send(const struct msg_block_t *msg_block, int32_t msg_block_num)
{
	...
	if (log_nonsec_ptr && cpu_id < CFG_TEE_CORE_NB_CORE) {
