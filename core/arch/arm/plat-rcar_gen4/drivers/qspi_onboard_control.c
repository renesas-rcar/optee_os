 ...
 */
	...
	io_write32((vaddr_t)RPC_DRCMR, 0x000C0000U);
	/*
	 * bit23-16 CMD[7:0] = 0x0C : 4FAST_READ 0Ch
	 */
	DREAR_value = (read_ext_top_addr >> EXT_ADDR_BIT_SHIFT_9);
	io_write32((vaddr_t)RPC_DREAR, (DREAR_value | RPC_DREAR_EAC));
	/*
	...
	io_write32((vaddr_t)RPC_DRENR, 0x0000CF00U);
	/*
						...
						uint32_t write_data_addr)
{
	...
	io_write32((vaddr_t)RPC_SMCMR, 0x00120000U);
	/*
	...
	io_write32((vaddr_t)RPC_SMENR, 0x00004F0FU);
	/*
	 * bit31-30 CDB[1:0]   =   00 : 1bit width command (QSPI0_MOSI)
	 * bit25-24 ADB[1:0]   =   00 : 1bit width address (QSPI0_MOSI)
	 * bit17-16 SPIDB[1:0] =   00 : 1bit width transfer data (QSPI0_MISO)
	...
	if (ret == FL_DRV_OK) {
		io_write32((vaddr_t)RPC_PHYCNT, 0x00030270U);
		/*
